...
```bash
# Run report runtime smoke test
node scripts/test_report.mjs

# Run function-level JSON testcases
node scripts/test_cases.mjs .

# Enable report log when running skill locally
report_mode=local node scripts/test_report.mjs
```

If viewer tools were selected during generation, you can also run:
...
When using the scaffold script directly, the corresponding parameters are:
