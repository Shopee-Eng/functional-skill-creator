...
```bash
# 运行 report runtime 冒烟测试
node scripts/test_report.mjs

# 运行 function 级 JSON testcase
node scripts/test_cases.mjs .

# 本地运行 skill 时开启 report log
report_mode=local node scripts/test_report.mjs
```

若生成时选择了 viewer 工具，还可以运行：

```bash
node tools/log_viewer.mjs .
node tools/tester_viewer.mjs .
```

直接使用 scaffold 脚本时，对应参数为：

```bash
node skills/fskill-creator/scripts/init_skill.mjs skills/my-skill \
  --include-report true \
  --include-unittest true \
  --include-viewers false
```
