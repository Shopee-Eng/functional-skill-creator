...
通过 [skills.sh](https://skills.sh/) / Skills CLI 安装（支持 Cursor、Claude Code、Codex、PromptScript 等 [70+ agent](https://github.com/vercel-labs/skills#supported-agents)）：

```bash
# 项目级安装（默认；PromptScript 等 agent 必须用这种方式）
npx skills add Shopee-Eng/functional-skill-creator --skill fskill-creator -y
```

仅当你的 agent 支持全局安装时再加 `-g`（如 Cursor、Claude Code）。**PromptScript 不支持 `-g`**，会报 `does not support global skill installation`。

指定 agent 时可重复传入 `-a`，例如 `-a cursor -a claude-code -a promptscript`。
